...
class _FeelingsScreenState extends ConsumerState<FeelingsScreen> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          // Listen — reactive play/pause (hidden if audio URL didn't resolve)
          if (audioUrl != null)
  ...
  Future<void> _selectFeeling(FeelingAyah feeling) async {
    ...
    try {
      ...
      // Audio resolution mirrors the daily-ayah provider: try the QF
      // recitations endpoint first, fall back to cdn.islamic.network
      // (which has working per-ayah files for all 6 reciters) so audio
      // still works when the QF call is auth-gated or unavailable.
      String? audioUrl;
      try {
        audioUrl = await quranApi.getAudioUrl(reciterId, ayah.verseKey);
      } catch (_) {
        final absAyah = absoluteAyahFromVerseKey(ayah.verseKey);
        if (reciterPath.isNotEmpty && absAyah != null) {
          audioUrl = islamicNetworkAyahUrl(reciterPath, absAyah);
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(AppTranslations.get('could_not_load', lang)),
